...
// Modify the compareFaces function to return a promise
function compareFaces(sourceImage, targetImage) {
      ...
      recognition.compareFaces(params, (err, data) => {
        if (err) {
          ...
          if (data.FaceMatches.length > 0) {
            ...
            resolve(`Faces are similar with similarity score: ${similarity}`); // Resolve the promise with the result
          } else {
  ...
  const CompareFaces = async (req, res) => {
    try {
      ...
      const data = await compareFaces('Krishna.jpg', req.file.buffer);
